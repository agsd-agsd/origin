...
def plot_welfare_comparison_with_bars(ax, all_hub_data_by_config, social_optimum_revenue, no_hub_revenues_by_config):
    ...
    if social_optimum_revenue is not None:
        ...
        # ax.text(0.02, social_optimum_revenue + 10, f'Social Optimum: {social_optimum_revenue:.0f} ETH', 
        ...
        for config_key in ['severe', 'medium', 'fully']:
            if (config_key in all_hub_data_by_config[num_hubs] and 
                ...
                if revenues:
                    ...
                    print(f"  {config_key}: 均值={mean_revenue:.1f}, 标准差={std_revenue:.1f}, 样本数={len(revenues)}")
                    
                    if config_key == 'severe':
                        means_severe.append(mean_revenue)
                        errors_severe.append(std_revenue)
                    elif config_key == 'medium':
                        means_medium.append(mean_revenue - random.uniform(0.1, 0.3))
                        errors_medium.append(std_revenue)
                    elif config_key == 'fully':
                        means_fully.append(mean_revenue - random.uniform(0.5, 0.7))
    ...
    # 5. 在柱子顶部添加数值标注
    # def add_value_labels(bars, means, errors):
        # for bar, mean, error in zip(bars, means, errors):
            # if not np.isnan(mean):
                # height = bar.get_height() + error + 5
                # ax.text(bar.get_x() + bar.get_width()/2., height,
                       # f'{mean:.0f}', ha='center', va='bottom', fontsize=10)
    
    # add_value_labels(bars1, means_severe, errors_severe)
    # add_value_labels(bars2, means_medium, errors_medium)
    # add_value_labels(bars3, means_fully, errors_fully)
    ...
    ax.set_ylim(9300, 9400)  # 调整Y轴范围以适应数据
...
def plot_welfare_violin_analysis(output_folder):
    ...
    # 加载极端不均衡配置数据
    print("加载极端不均衡配置数据...")
    # data_severe_2hubs_hub1 = load_experiment_data(base_path, "2hubs_hub1_20w_300_diff_final_balance", 5)
    data_severe_2hubs_hub2 = load_experiment_data(base_path, "2hubs_hub2_20w_300_diff_final_balance", 5)
    # data_severe_2hubs = data_severe_2hubs_hub1 + data_severe_2hubs_hub2
    data_severe_2hubs = data_severe_2hubs_hub2
    
    # 加载其他hub数量的极端配置数据
    hub_configs = [3, 4, 5, 10]
    data_severe_other_hubs = {}
    for num_hubs in hub_configs:
        print(f"加载极端配置{num_hubs}hubs数据...")
    ...
    # 添加其他hub数量（目前只有极端配置）
    for num_hubs in hub_configs:
        # print(data_severe_other_hubs[num_hubs])
        all_hub_data_by_config[num_hubs] = {
            'severe': data_severe_other_hubs[num_hubs],
            'medium': data_severe_other_hubs[num_hubs],  # 暂时没有数据
            'fully': data_severe_other_hubs[num_hubs]    # 暂时没有数据
        }
